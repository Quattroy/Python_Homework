...
number_berries = [random.randint(1, 100) for i in range(N)]
print(number_berries)
max = number_berries[0] + number_berries[1] + number_berries[2]
print(max)
for i in range(len(number_berries)-2):
    new_max = number_berries[i] + number_berries[i+1]+ number_berries[i+2]
    if max < new_max:
        max = new_max
print(f"Максимальное число ягод, которое может собрать модуль за один ход составляет: {max}")